...
else if($a == 'reg')
{
    ...
    $r = $dosql->GetOne("SELECT `id` FROM `#@__member` WHERE `cardnum`='cardnum'");
    if(isset($r['id']))
    {
        ShowMsg('您填写的身份证已被注册！','?c=reg');
        exit();
    }
	...
	$sql = "INSERT INTO `#@__member` (username, password, email, cardnum,expval, regtime, regip, logintime, loginip) VALUES ('$username', '$password', '$email','$cardnum', '0', '$regtime', '$regip', '$regtime', '$regip')";
...
{
	require(PHPMYWIND_TEMP.'/default/member/friend.php');
	exit();
}
else if($c == 'order')
{
