...
?>
<div class="gray_header"> <span class="title">修改管理组</span> <span class="reload"><a href="javascript:location.reload();">刷新</a></span></div>
<form name="form" id="form" method="post" action="admingroup_save.php" onsubmit="return cfm_admingroup();">
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="form_table">
		<tr>
			...
			<td width="75%"><input type="text" name="groupname" class="class_input" id="groupname" value="<?php echo $row['groupname']; ?>" />
		...
		<tr>
			...
			if($id != 1)
			{
			?><div class="agtitle"><strong>网站系统管理</strong></div>
				...
				<div class="agline"> <span>
					...
					开发帮助</span></div>
				<div class="agsel"><a href="javascript:;" onclick="SelModel(true)">全选</a>&nbsp;&nbsp;<a href="javascript:;" onclick="SelModel(false)">反选</a></div>
			<?php
			}
			else
			{
		...
		<tr class="nb">
			...
			if($id != 1)
			{
				...
				{
					echo '<div class="agtitle"><strong>'.$row2['sitename'].'</strong></div>';
					Show($row2['id'],$row['id']);
					echo '<div class="agsel"><a href="javascript:;" onclick="SelPriv('.$row2['id'].',true)">全选</a>&nbsp;&nbsp;<a href="javascript:;" onclick="SelPriv('.$row2['id'].',false)">反选</a></div>';
					$i++;
				}
			}
			else
...
<div rel="rowpid_<?php echo $topid ; ?>">
	<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table">
		<tr align="center" class="mgr_tr">
			<td width="3%" height="32" align="left"><input type="checkbox" name="siteid[<?php echo $siteid; ?>]" value="<?php echo $siteid; ?>" onclick="SelRole(<?php echo $siteid; ?>,<?php echo $row['id']; ?>,this);" <?php echo $cktop; ?> /></td>
			<td align="left"><?php
			for($n=1; $n<$i; $n++) echo '&nbsp;&nbsp;';
