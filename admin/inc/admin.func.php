...
function CategoryType($type=0, $id=0, $i=0)
{
	...
	while($row = $dosql->GetArray($id))
	{
		...
		if($_SESSION['adminlevel'] != 1)
		{
			...
			{
				$r2 = $dosql->GetOne("SELECT * FROM `#@__adminprivacy` WHERE `groupid`=".$_SESSION['adminlevel']." AND `model`='category' AND `classid`=".$row['id']." AND `action`='add'");
			}
...
function IsModelPriv($m='')
{
	...
	if($_SESSION['adminlevel'] != 1)
	{
		$r = $dosql->GetOne("SELECT * FROM `#@__adminprivacy` WHERE `groupid`=".$_SESSION['adminlevel']." AND `model`='$m'");
		if(isset($r) && is_array($r))
		{
			return TRUE; 
		}
...
function IsCategoryPriv($cid=0,$act='',$return='')
{
	...
	if($_SESSION['adminlevel'] != 1)
	{
		$r = $dosql->GetOne("SELECT * FROM `#@__adminprivacy` WHERE `groupid`=".$_SESSION['adminlevel']." AND `model`='category' AND `classid`=$cid AND `action`='$act'");
		if(isset($r) && is_array($r))
		{
			return TRUE; 
		}
